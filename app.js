const tlLeave = gsap.timeline({duration: 0.75, ease : 'Power2.easeOut'});
const tlEnter = gsap.timeline({duration: 0.75, ease : 'Power2.easeOut'});

// Function tl for leave and enter animation
...
const leaveAnimation = (current, done) => {
    ...
    return (
        tlLeave.fromTo(arrow, {opacity: 1, y: 0}, {opacity: 0, y: 40}),
...
const enterAnimation = (current, done, gradient) => {
  ...
  const arrow = current.querySelector('.showcase-arrow');
  return (
      tlEnter.fromTo(arrow, {opacity: 0, y: 40}, {opacity: 1, y: 0}),
